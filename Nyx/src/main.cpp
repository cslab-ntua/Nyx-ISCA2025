...
int main(int argc, char **argv){
	...
	std::cout << "[INFO]: ****** Launching Nyx example ******" << std::endl;
	std::cout << std::endl;
	...
	#if defined(NONSHARING) || defined(SUBOPT_NONSHARING)
	...
	while(completed_apps.size()<MAX_APPS){
		...
		launch_fcfs(active_regions, sel_prr);
		icap = pr_task(sel_prr, sel_task);
		nonshare_scheduling(pending_apps, baseline_app, baseline_tasks);
		...
		if(icap){
			...
			sel_task = select_task_nimblock(candidate_apps, sel_prr);
		}
		...
		#elif defined (SUBOPT_NONSHARING)

		is_completed(baseline_app, completed_apps);

		if(icap){
			sel_prr = select_prr_subopt(active_regions);
			sel_task = select_task_subopt(baseline_tasks, sel_prr);
		}
		launch_subopt(active_regions, sel_prr);
		icap = pr_task(sel_prr, sel_task);
		subopt_scheduling(pending_apps, baseline_app, baseline_tasks);
	...
	std::cout << std::endl;
	std::cout <<"[INFO]: ****** Nyx has finished ******" << std::endl;
