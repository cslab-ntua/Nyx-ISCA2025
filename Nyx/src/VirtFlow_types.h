...
static double pr_latency = 0.00598;

// Low Demand 1500ms = 1.5s
//Stressful Conditions 200ms = 0.2s
...
class Task {
	...
	public:
		...
		Task(int id, const char* name, Application* app_id, double latency, uint32_t max_batch, int state, const char** bin):
		id(id),
		name(name),
		app_id(app_id),
		latency(latency),
...
class PRRegion{
	public:
		...
		PRRegion(int id, int status, uint64_t vfifo_ctrl_addr, uint64_t decouple_addr, uint64_t axilite_addr):
		id(id),
		status(status),
		vfifo_ctrl_addr(vfifo_ctrl_addr),
