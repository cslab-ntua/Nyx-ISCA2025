...
/* select_prr_sjf should be modified. The selection now is based on a random slot metric. Might need something more efficient */
...
Task* select_task_sjf(std::list<Task*> &waiting_tasks, PRRegion* sel_prr){
	...
	if(waiting_tasks.empty()==true || sel_prr==NULL){
		...
		ret_task->src = sel_prr->src_addr;

		for(int iter=0; iter<curr_dep; iter++){
			if(curr_app->id == 1){
				int tmp_ = curr_app->tasks[curr_app->dep_list[ret_task->id][iter]]->task_rdy;
				curr_app->tasks[curr_app->dep_list[ret_task->id][iter]]->dest[iter+tmp_] = sel_prr->dest_addr[iter+tmp_];
				curr_app->tasks[curr_app->dep_list[ret_task->id][iter]]->task_rdy +=1;
			}
			else{
				int tmp_ = curr_app->tasks[curr_app->dep_list[ret_task->id][iter]]->task_rdy;
				curr_app->tasks[curr_app->dep_list[ret_task->id][iter]]->dest[tmp_] = sel_prr->dest_addr[tmp_];
				curr_app->tasks[curr_app->dep_list[ret_task->id][iter]]->task_rdy +=1;
			}
		}
