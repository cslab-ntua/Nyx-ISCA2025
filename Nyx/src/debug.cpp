...
#endif
{
        ...
        std::string file_name = "nyx_nonsharing_" + std::to_string(interval) + ".csv";
        outfile.open(file_name.c_str(), std::ios_base::app);
        #elif defined(FCFS)
        std::string file_name = "nyx_fcfs_" + std::to_string(interval) + ".csv";
        outfile.open(file_name.c_str(), std::ios_base::app);
        #elif defined(RR)
        std::string file_name = "nyx_rr_" + std::to_string(interval) + ".csv";
        outfile.open(file_name.c_str(), std::ios_base::app);
        #elif defined(SJF)
        std::string file_name = "nyx_sjf_" + std::to_string(interval) + ".csv";
        outfile.open(file_name.c_str(), std::ios_base::app);
        #elif defined(NIMBLOCK)
 	std::string file_name = "nyx_nimblock_" + std::to_string(interval) + ".csv";
        outfile.open(file_name.c_str(), std::ios_base::app);
        #endif
		...
		double total_reconfig = 0.000f;
                ...
                outfile << app->app_name << "_" << app->max_batch << ",";
        ...
        std::ofstream outfile;
        std::string file_name = "nyx_app.csv";
        outfile.open(file_name.c_str(), std::ios_base::app);
	...
	//outfile << "Application Name" << "," << "Batch Size" << "," << "Response Time" << "," << std::endl;
        ...
        for(Application* app : completed_apps){
                ...
                outfile << duration(app->arrival_time,app->end_time) << ",";
                outfile << std::endl;
               /* outfile << "Task Name" << "," << "Execution Time" << "," << std::endl;

                double exec_time = 0.000f;
                //double total_exec = 0.000f;
                for(Task* task: app->tasks){
                        outfile << task->name << ",";
                        exec_time = duration(task->start_time, task->end_time);
