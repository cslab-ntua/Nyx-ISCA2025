...
void padImage(DTYPE *src, stream256 &dst,  int srcm,  int srcn,  int dstm,  int dstn){
	...
	#pragma HLS INTERFACE mode=m_axi bundle=gmem depth=2073600 port=src offset=slave
	#pragma HLS INTERFACE mode=axis register_mode=both depth=2073600 port=dst register
...
void extractBlockD(stream256 &src, stream256 &dst,  int srcm,  int srcn){
	...
	#pragma HLS INTERFACE mode=axis register_mode=both depth=2073600 port=src register
	#pragma HLS INTERFACE mode=axis register_mode=both depth=2073600 port=dst register
	#pragma HLS INTERFACE s_axilite port=srcm bundle=control
	#pragma HLS INTERFACE s_axilite port=srcn bundle=control
	#pragma HLS interface s_axilite port=return bundle=control
				...
				if(j<srcn){
					loop3:for(int k=0; k<8; k++){
						loop5: for(int l=0; l<8; l++){
							#pragma HLS PIPELINE
							bit256 tmp_ = src.read();
							//buffer[k][l] = src[((i+l)*srcn + j) + k];
							buffer[k][l] = (DTYPE)tmp_;
						}
					}
...
void reconstructBlockD(stream256 &src, DTYPE *dst,  int srcm,  int srcn){
	#pragma HLS INTERFACE mode=axis register_mode=both depth=2073600 port=src register
	#pragma HLS INTERFACE mode=m_axi bundle=gmem depth=2073600 port=dst offset=slave
