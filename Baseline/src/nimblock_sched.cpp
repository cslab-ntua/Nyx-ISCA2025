...
bool is_ready(Task* task){
	...
	for(uint32_t iter=0; iter<num_deps; iter++){
		//if(task->compl_batch < parent_app->tasks[parent_app->dep_list[task->id][iter]]->compl_batch)
 ...
 */
Task* select_task_nimblock(std::list<Application*>& candidate_apps){
    ...
    for (auto it = target_app->tasks.begin(); it!=target_app->tasks.end(); ++it) {
        my_task = *it;
        // std::cout << "[DEBUG]: Task id:  " << my_task->id  << std::endl;
        if (is_ready(my_task) && (my_task->state == TASK_PENDING)) {
            
            
             int curr_dep = target_app->num_deps[my_task->id];
            
             for(int iter=0; iter<curr_dep; iter++){
                 target_app->tasks[target_app->dep_list[my_task->id][iter]]->task_rdy +=1;
             }
...
void launch_nimblock(std::vector<PRRegion*> &regions, PRRegion* sel_prr){
	...
	for(PRRegion* prr : regions){
		if((prr->status == PR_CONFIG || prr->status == PR_WAITING) && prr->id!=sel){
			...
			//if(curr_dep_rev==curr_task->task_rdy){ // den mpainei edw
				int flag=0;
...
// Changed accordingly to fit our case
/* 
 * Returns the slot to preempt to if an application has overused its resource allocation
 */
PRRegion* preempt_slot_apps(std::vector<PRRegion*> regions, Task* my_task) {
    // TODO is it better to just preempt anything right away???
    int largest_diff = 0;
    PRRegion* diff_slot = NULL;
    for (auto it = regions.begin(); it != regions.end(); ++it) {
        // pick slot that exceeds its threshold by the most to take away slots
        PRRegion* cur_slot = *it;
        if (cur_slot ->status == PR_WAITING) {
            int diff = cur_slot->curr_task->app_id->occupancy - cur_slot->curr_task->app_id->threshold;
            if (diff > largest_diff) {
                ...
                // same application
                if(cur_slot->curr_task->app_id == diff_slot->curr_task->app_id){
                    if (cur_slot->curr_task->id > diff_slot->curr_task->id) {
        ...
        if (diff_slot != NULL && cur_slot->status == PR_RUNNING) {
            if (diff_slot->curr_task->app_id == cur_slot->curr_task->app_id) {
                if(cur_slot->curr_task->id > diff_slot->curr_task->id) {
                    diff_slot = NULL;
                }
            }
